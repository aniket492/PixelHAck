document.addEventListener('DOMContentLoaded', () => {
    ...
    if (grid) {
        properties.forEach(prop => {
            ...
            wrapper.style.gridColumn = `span ${prop.span}`;
            
            const card = document.createElement('div');
            card.className = 'property-card';
            card.style.backgroundImage = `url(${prop.img})`;
            
            const info = document.createElement('div');
            info.className = 'property-card-info';
            info.innerHTML = `<h3>${prop.title}</h3><span>${prop.location}</span>`;
    ...
    // --- 5. PHILOSOPHY MARQUEE (from Page 6) ---
    const philosophies = [
    ...
    if (marqueeTrack) {
        const allCards = [...philosophies, ...philosophies]; // Duplicate for seamless loop
